...
from ficsit2.chain.recipe_select import convert_user_dict_to_recipes, find_recipe_by_name, user_pick_recipe
...
class ChainGraph():
    ...
    final_recipes: List[Tuple[ComponentName, str]] = field(init=False, default_factory=list)

    

    def __post_init__(self):
        self.total_values = TotalProductionValues()
        self.recipes_selected = convert_user_dict_to_recipes(self.all_recipes, self.recipes_selected)

        self.root = ComponentNode(self.initial_component, 1, mod_content=self.mod_content)
    ...
    def add_selected_component_recipes(self, components:List[ComponentNode]):
        ...
        for component in components:
            ...
            self.add_selected_component_recipes(component.node_children[0].node_children)
    ...
    def add_selected_recipe(self, component: ComponentNode):
        ...
        self.final_recipes.append((component.name, child_recipe.name, component.node_depth))
    ...
    def _get_recipe(self, component:ComponentNode)  -> dict:
        ...
        if self.use_standard:
            return find_recipe_by_name(self.all_recipes[component.name.value], "Standard")

        if component.node_depth in self.recipes_selected.keys():
            recipe =  self.recipes_selected[component.node_depth].get(component.name.value)

        if recipe is None:
            for component_name, recipe_name, _ in self.final_recipes:
                if component.name == component_name:
                    return find_recipe_by_name(self.all_recipes[component.name.value], recipe_name)

            recipe, self.use_standard = user_pick_recipe(component.name.value, self.all_recipes)
            return recipe
    ...
    def str_comparison(self):
        ...
        all_recipes_needed = list(set([f"\t{selection[1]} for {selection[0].value}"for selection in self.final_recipes]))
        return (
